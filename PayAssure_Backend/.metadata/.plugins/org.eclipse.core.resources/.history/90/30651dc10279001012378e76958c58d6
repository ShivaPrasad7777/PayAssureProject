...
//to remove the check status cond
//package com.example.controller;
//
//import com.example.dto.CustomerSummary;
//import com.example.dto.PolicySummary;
...
// return ResponseEntity.ok(customers);
//}
////NEW: Fetch unpaid and failed invoices for a customer
//@GetMapping("/customers/{customerId}/unpaid-failed-invoices")
//public ResponseEntity<List<Invoice>> getUnpaidFailedInvoicesForCustomer(@PathVariable String customerId) {
// List<Invoice> invoices = billingService.getUnpaidFailedInvoicesForCustomer(customerId);
// return ResponseEntity.ok(invoices);
...
//    // Create invoice (handles multi-policy and returns list)
//    @PostMapping("/invoices")
//    public ResponseEntity<List<Invoice>> createInvoice(@RequestBody InvoiceRequest request) {
//        try {
//            List<Invoice> invoices = billingService.createInvoice(
//                    request.getCustomerId(),
//                    request.getPolicyIds(),
//                    request.getInsurerId(),
//                    request.getDueDate(),
...
//            );
//            if (invoices.isEmpty()) {
//                return new ResponseEntity<>(HttpStatus.NO_CONTENT);  // 204 if none generated
//            }
//            return new ResponseEntity<>(invoices, HttpStatus.CREATED);
//        } catch (RazorpayException e) {
//            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
//        } catch (RuntimeException e) {
//            return new ResponseEntity<>(List.of(), HttpStatus.BAD_REQUEST);  // Empty list with 400
//        }
//    }
//
//    // Initiate payment
