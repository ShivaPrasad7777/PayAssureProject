...
function StatCard({ title, count, color, icon, image }) {
  return (
    ...
    >
      ...
      <CardContent sx={{ textAlign: "center", pt: 2, px: 2, flexGrow: 1 }}>
        <Typography variant="h4" sx={{ color, fontWeight: 700, mb: 1 }}>
          {count}
        </Typography>
        <Typography variant="subtitle1" sx={{ color: "#222", fontWeight: 600 }}>
          {title}
        </Typography>
      </CardContent>
    </Card>
...
function getPolicyIconAndImage(policyName = "") {
  ...
  if (lower.includes("home")) return { icon: <HomeIcon />, image: IMAGE_URLS.home };
  if (lower.includes("auto") || lower.includes("car")) return { icon: <DirectionsCarIcon />, image: IMAGE_URLS.auto };
  if (lower.includes("business") || lower.includes("commercial")) return { icon: <BusinessIcon />, image: IMAGE_URLS.business };
  if (lower.includes("health") || lower.includes("medical")) return { icon: <FavoriteIcon />, image: IMAGE_URLS.health };
  if (lower.includes("travel") || lower.includes("trip")) return { icon: <FlightIcon />, image: IMAGE_URLS.travel };
...
export default function CustomerDashboard({ user }) {
  ...
  const enrichInvoicesWithPolicyNames = (invoices, policiesList) => {
    return invoices.map((invoice) => {
      let ids = [];
      if (Array.isArray(invoice.policyIds)) ids = invoice.policyIds;
      else if (invoice.policyId) ids = [invoice.policyId];
      ids = [...new Set(ids)];
      const names = ids
        .map((id) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([policyRes, paymentRes, invoiceRes, customerRes]) => {
        ...
        setUnpaidInvoices(enrichInvoicesWithPolicyNames(invoicesList, policiesList));
        setCustomer(customerRes.data);

        // Payments enrichment
        const paymentsRaw = Array.isArray(paymentRes.data) ? paymentRes.data : [];
        setPayments(paymentsRaw); // You can enrich further as in your previous code if needed
      })
      .catch((err) => {
